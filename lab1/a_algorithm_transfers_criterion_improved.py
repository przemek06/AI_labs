...
def h_eval(node, end_node):
    coords_1 = (float(node.longitude), float(node.latitude))
    coords_2 = (float(end_node.longitude), float(end_node.latitude))

    distance_cost = (geopy.distance.geodesic(coords_1, coords_2).km ** 2) * 1/128
...
def a_algorithm_transfers_criterion_improved(stop_a, stop_b, start_time, graph):
    benchmark_time = time()
    start_nodes = get_start_node(stop_a, graph, start_time)
    end_node = get_end_node(stop_b, graph)
    ...
    while len(open) > 0:
        ...
        for next_departure in neigbour_departures:
            if (next_departure[6], next_departure[2]) not in closed.keys() and (next_departure[6], next_departure[2]) not in open.keys():
                next_node = construct_next_node(min_node, next_departure, end_node)
